...
function buildGatewayHeaders(
  ...
  // For POST/PUT requests, always ensure Content-Length is provided
  if (requestBody) {
    cleaned['content-length'] = Buffer.byteLength(requestBody, 'utf8').toString();
  } else if (['POST', 'PUT', 'PATCH'].includes(headers?.method || 'POST')) {
...
async function forwardToGateway(payload: Required<Pick<ProxyPayload, 'url' | 'method'>> & ProxyPayload) {
  const target = new URL(payload.url);
  const requestBody = payload.body ?? undefined;
  const headers = buildGatewayHeaders(payload.headers, target, requestBody);

  // Use Undici's fetch with custom dispatcher to bypass self-signed certs
  ...
  response.headers.forEach((value, key) => {
    if (key.toLowerCase() === 'set-cookie') {
      // undici headers.forEach yields set-cookie multiple times or joined, safely extract them
  ...
  // Extract set-cookie natively if possible
  const rawSetCookies = response.headers.getSetCookie ? response.headers.getSetCookie() : setCookies;
...
export async function POST(request: Request) {
  ...
  try {
    // Merge browser native headers (User-Agent, Accept, Cookie) into the payload headers
    payload.headers = payload.headers || {};
    
    // Safely forward standard browser headers to prevent WAF/Gateway rejections
    const safeHeadersToForward = ['user-agent', 'accept', 'accept-language', 'accept-encoding'];
    for (const headerName of safeHeadersToForward) {
      const headerVal = request.headers.get(headerName);
      if (headerVal && !payload.headers[headerName]) {
        payload.headers[headerName] = headerVal;
      }
    }
